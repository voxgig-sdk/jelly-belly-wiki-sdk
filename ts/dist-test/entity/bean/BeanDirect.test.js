...
(0, node_test_1.describe)('BeanDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.JellyBellyWikiSDK({
    ...
    (0, node_test_1.test)('direct-load-bean', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-bean', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-bean', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-bean', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.JELLY_BELLY_WIKI_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.JellyBellyWikiSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['JELLY_BELLY_WIKI_TEST_BEAN_ENTID'];
    ...
    const client = new __1.JellyBellyWikiSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=BeanDirect.test.js.map