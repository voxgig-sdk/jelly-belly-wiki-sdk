...
describe('RecipeEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'recipe.' + op, live)) return
    }
    ...
    const recipe_ref01_list = await recipe_ref01_ent.list(recipe_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['JELLY_BELLY_WIKI_TEST_RECIPE_ENTID']
