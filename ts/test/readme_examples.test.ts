...
//     (\`new JellyBellyWikiSDK(...)\` / \`JellyBellyWikiSDK.test(...)\` become
//     \`JellyBellyWikiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'JellyBellyWikiSDK'
...
const TEST_SEED = {"entity":{"bean":{"test01":{"id":"test01"}},"combination":{"test01":{"id":"test01"}},"fact":{"test01":{"id":"test01"}},"history":{"test01":{"id":"test01"}},"recipe":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new JellyBellyWikiSDK(...)\` and any
// \`JellyBellyWikiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+JellyBellyWikiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/JellyBellyWikiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
