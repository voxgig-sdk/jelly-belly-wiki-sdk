...
type Args struct {
	Entity string         `json:"entity" jsonschema:"bean | combination | fact | history | recipe"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.JellyBellyWikiSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "jelly-belly-wiki_list",
		Description: "List records from JellyBellyWiki. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "jelly-belly-wiki_load",
		Description: "Load a single record from JellyBellyWiki. " +
...
func runOp(client *sdk.JellyBellyWikiSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.JellyBellyWikiSDK, name string) (sdk.JellyBellyWikiEntity, error) {
	switch strings.ToLower(name) {
	case "bean":
		return client.Bean(nil), nil
	case "combination":
		return client.Combination(nil), nil
	case "fact":
		return client.Fact(nil), nil
	case "history":
