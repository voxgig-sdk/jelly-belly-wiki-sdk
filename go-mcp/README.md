# jelly-belly-wiki-mcp

[MCP](https://modelcontextprotocol.io) server exposing the JellyBellyWiki SDK as
two agent tools — `jelly-belly-wiki_list` and `jelly-belly-wiki_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jelly-belly-wiki-mcp)
make build

# 2. Provide credentials via the environment
export JELLY_BELLY_WIKI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user jelly-belly-wiki \
  -- /absolute/path/to/jelly-belly-wiki-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./jelly-belly-wiki-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// jelly-belly-wiki_list: first page of records
{ "entity": "bean" }
{ "entity": "bean", "query": { } }

// jelly-belly-wiki_load: one record by id
{ "entity": "bean", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jelly-belly-wiki-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user jelly-belly-wiki \
     -- "$PWD"/dist/*/jelly-belly-wiki-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `jelly-belly-wiki_list` and `jelly-belly-wiki_load` tools now appear
   in new sessions. Ask the agent to *"list bean using jelly-belly-wiki"*
   and it calls `jelly-belly-wiki_list` with `{"entity":"bean"}`.
...
```sh
export JELLY_BELLY_WIKI_APIKEY=sk_live_xxx            # API key
export JELLY_BELLY_WIKI_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `jelly-belly-wiki_list` tool
...
### Call the `jelly-belly-wiki_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `jelly-belly-wiki_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `jelly-belly-wiki_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `JELLY_BELLY_WIKI_APIKEY` | API key sent with every request. |
| `JELLY_BELLY_WIKI_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

bean | combination | fact | history | recipe

### Smoke test via HTTP (raw JSON-RPC)

```sh
./jelly-belly-wiki-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"jelly-belly-wiki_load","arguments":{"entity":"bean","query":{"id":1}}}}'
```
