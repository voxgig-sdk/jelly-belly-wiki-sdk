...
The PHP SDK for the JellyBellyWiki API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Bean(null)->list(null, null);
...
```php
[$result, $err] = $client->Bean(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->JellyBellyWiki(null)->load(
    ["id" => "test01"], null
);
