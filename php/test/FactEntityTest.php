...
class FactEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = JellyBellyWikiSDK::test(null, null);
        $ent = $testsdk->Fact(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Fact(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Fact(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = fact_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "fact." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set JELLY_BELLY_WIKI_TEST_FACT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $fact_ref01_data = null;
        if (count($fact_ref01_data_raw) > 0) {
...
function fact_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/fact/FactTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["fact01", "fact02", "fact03"] as $k) {
    ...
    $entid_env_raw = getenv("JELLY_BELLY_WIKI_TEST_FACT_ENTID");
