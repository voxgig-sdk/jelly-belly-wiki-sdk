...
// JellyBellyWiki SDK feature test
//
...
class FtCtrl extends JellyBellyWikiControl
{
...
class FtClient
{
    ...
    private JellyBellyWikiUtility $utility;

    public function __construct(JellyBellyWikiUtility $utility, array $options)
    {
    ...
    public function get_utility(): JellyBellyWikiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JellyBellyWikiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public JellyBellyWikiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (JellyBellyWikiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JellyBellyWikiConfig::make_feature($name);
    ...
    private static function buildUrl(JellyBellyWikiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JellyBellyWikiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JellyBellyWikiResponse($fetched);
        }
    ...
    private function fail(JellyBellyWikiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JellyBellyWikiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
