...
describe("BeanDirect", function()
  it("should direct-list-bean", function()
    local setup = bean_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-bean", setup.live and "live" or "unit")
  ...
  it("should direct-load-bean", function()
    local setup = bean_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-bean", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function bean_direct_setup(mockres)
