...
The Lua SDK for the JellyBellyWiki API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Bean(nil):list(nil, nil)
...
```lua
local result, err = client:Bean(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:JellyBellyWiki(nil):load(
  { id = "test01" }, nil
)
