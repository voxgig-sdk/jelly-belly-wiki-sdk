...
local RecipeEntity = {}
RecipeEntity.__index = RecipeEntity


function RecipeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RecipeEntity)
  self._name = "recipe"
  self._client = client
...
function RecipeEntity:get_name()
  return self._name
end


function RecipeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RecipeEntity.new(self._client, opts)
end
...
function RecipeEntity:mark_deleted()
  self._deleted = true
end


function RecipeEntity:deleted()
  return true == self._deleted
end


function RecipeEntity:data_set(args)
  if args ~= nil then
...
function RecipeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RecipeEntity:match_set(args)
  if args ~= nil then
...
function RecipeEntity:match_get()
...
function RecipeEntity:stream(action, args, callopts)
...
---@param reqmatch RecipeListMatch
---@param ctrl? table
---@return Recipe[]
---@return string? err
function RecipeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RecipeEntity:_run_op(ctx, post_done)
  local utility = self._utility
