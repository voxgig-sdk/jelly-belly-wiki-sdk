...
local BeanEntity = {}
BeanEntity.__index = BeanEntity


function BeanEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BeanEntity)
  self._name = "bean"
  self._client = client
...
function BeanEntity:get_name()
  return self._name
end


function BeanEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BeanEntity.new(self._client, opts)
end
...
function BeanEntity:mark_deleted()
  self._deleted = true
end


function BeanEntity:deleted()
  return true == self._deleted
end


function BeanEntity:data_set(args)
  if args ~= nil then
...
function BeanEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BeanEntity:match_set(args)
  if args ~= nil then
...
function BeanEntity:stream(action, args, callopts)
...
function BeanEntity:_run_op(ctx, post_done)
  local utility = self._utility
