...
local FactEntity = {}
FactEntity.__index = FactEntity


function FactEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FactEntity)
  self._name = "fact"
  self._client = client
...
function FactEntity:get_name()
  return self._name
end


function FactEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FactEntity.new(self._client, opts)
end
...
function FactEntity:mark_deleted()
  self._deleted = true
end


function FactEntity:deleted()
  return true == self._deleted
end


function FactEntity:data_set(args)
  if args ~= nil then
...
function FactEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FactEntity:match_set(args)
  if args ~= nil then
...
function FactEntity:stream(action, args, callopts)
...
function FactEntity:_run_op(ctx, post_done)
  local utility = self._utility
