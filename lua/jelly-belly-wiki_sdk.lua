...
require("jelly-belly-wiki_types")
...
local JellyBellyWikiSDK = {}
JellyBellyWikiSDK.__index = JellyBellyWikiSDK
...
JellyBellyWikiSDK._make_feature = _make_feature


function JellyBellyWikiSDK.new(options)
  local self = setmetatable({}, JellyBellyWikiSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function JellyBellyWikiSDK:options_map()
...
function JellyBellyWikiSDK:get_utility()
  return Utility.copy(self._utility)
end


function JellyBellyWikiSDK:get_root_ctx()
  return self._rootctx
end


function JellyBellyWikiSDK:prepare(fetchargs)
...
function JellyBellyWikiSDK:direct(fetchargs)
...
-- by passing it.
function JellyBellyWikiSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function JellyBellyWikiSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "JellyBellyWikiSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function JellyBellyWikiSDK:Bean(data)
  ...
  if data == nil then
    if self._bean == nil then
      self._bean = EntityMod.new(self, nil)
    end
    return self._bean
  end
...
function JellyBellyWikiSDK:Combination(data)
  ...
  if data == nil then
    if self._combination == nil then
      self._combination = EntityMod.new(self, nil)
    end
...
function JellyBellyWikiSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = JellyBellyWikiSDK.new(sdkopts)
  sdk.mode = "test"
