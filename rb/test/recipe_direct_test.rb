...
class RecipeDirectTest < Minitest::Test
  def test_direct_list_recipe
    setup = recipe_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-recipe", setup[:live] ? "live" : "unit")
    if _should_skip
...
def recipe_direct_setup(mockres)
