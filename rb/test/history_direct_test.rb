...
class HistoryDirectTest < Minitest::Test
  def test_direct_list_history
    setup = history_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-history", setup[:live] ? "live" : "unit")
    if _should_skip
...
def history_direct_setup(mockres)
