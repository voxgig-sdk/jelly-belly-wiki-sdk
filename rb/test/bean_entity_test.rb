...
class BeanEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Bean(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Bean(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = bean_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "bean." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set JELLYBELLYWIKI_TEST_BEAN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    bean_ref01_data = nil
    if bean_ref01_data_raw.length > 0
      bean_ref01_data = Helpers.to_map(bean_ref01_data_raw[0][1])
    end

    # LIST
    bean_ref01_ent = client.Bean(nil)
    bean_ref01_match = {}

    bean_ref01_list_result = bean_ref01_ent.list(bean_ref01_match, nil)
...
def bean_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "bean", "BeanTestData.json")
  ...
  entid_env_raw = ENV["JELLYBELLYWIKI_TEST_BEAN_ENTID"]
