...
class CombinationDirectTest < Minitest::Test
  def test_direct_list_combination
    setup = combination_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-combination", setup[:live] ? "live" : "unit")
    if _should_skip
...
def combination_direct_setup(mockres)
