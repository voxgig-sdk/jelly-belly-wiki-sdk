...
module JellyBellyWikiUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = JellyBellyWikiContext.new({}, nil)
    end
    op = ctx.op || JellyBellyWikiOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || JellyBellyWikiResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(JellyBellyWikiError) ? err.msg : err.to_s
    ...
    sdk_err = JellyBellyWikiError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(JellyBellyWikiError)
