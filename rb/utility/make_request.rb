...
module JellyBellyWikiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = JellyBellyWikiResponse.new({})
    ...
    if fetch_err
      ...
      response = JellyBellyWikiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = JellyBellyWikiResponse.new(fetched)
    else
