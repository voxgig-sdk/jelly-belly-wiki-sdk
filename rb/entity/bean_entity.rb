...
class BeanEntity
  ...
  def make
    opts = @_entopts.dup
    BeanEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = JellyBellyWikiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Bean, Hash] the current Bean data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = JellyBellyWikiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Bean fields)
  def match_get
  ...
  # Load a single Bean.
  #
  # @param reqmatch [BeanLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Bean.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Bean, Hash] the loaded Bean; raises JellyBellyWikiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = JellyBellyWikiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [BeanListMatch, Hash, nil] match filter (any subset of
  #   Bean fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Bean>, Array] the matching Bean items; raises JellyBellyWikiError on failure
