...
The Ruby SDK for the JellyBellyWiki API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Bean(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.JellyBellyWiki(nil).load(
  { "id" => "test01" }, nil
)
