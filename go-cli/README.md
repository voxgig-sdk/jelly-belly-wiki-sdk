# jelly-belly-wiki-cli

boru-driven command-line client **and** interactive REPL for the JellyBellyWiki
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jelly-belly-wiki-cli)
make build

# 2. See usage (words, entities, env vars)
./jelly-belly-wiki-cli --help

# 3. Provide credentials once, via the environment
export JELLY_BELLY_WIKI_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./jelly-belly-wiki-cli list bean
./jelly-belly-wiki-cli load 1 bean            # {id:1} shorthand
./jelly-belly-wiki-cli load '{id:1}' bean       # explicit match map
./jelly-belly-wiki-cli list combination

# 5. Override the API base URL for a single call
JELLY_BELLY_WIKI_BASE=https://api.example.com ./jelly-belly-wiki-cli list bean

# 6. No arguments -> interactive REPL
./jelly-belly-wiki-cli
jelly-belly-wiki> list bean
jelly-belly-wiki> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jelly-belly-wiki-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/jelly-belly-wiki-cli list bean
   ```
...
```sh
./jelly-belly-wiki-cli load 1 bean          # scalar shorthand for {id:1}
./jelly-belly-wiki-cli load '{id:1}' bean     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `jelly-belly-wiki>`). Each line is
evaluated as its own boru expression:

```text
$ ./jelly-belly-wiki-cli
jelly-belly-wiki> list bean
jelly-belly-wiki> /help
jelly-belly-wiki> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `bean`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/jelly-belly-wiki-cli`. |
...
(`./jelly-belly-wiki-cli <expr>`) and interactively (the REPL), and expressions compose the
