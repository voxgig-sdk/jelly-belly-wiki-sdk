// Command JellyBellyWiki-cli is a CLI + REPL for the JellyBellyWiki SDK,
...
const prompt = "jelly-belly-wiki"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "bean combination fact history recipe"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      JELLY_BELLY_WIKI_APIKEY (api key), JELLY_BELLY_WIKI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: JELLY_BELLY_WIKI_APIKEY carries the API key and
	// JELLY_BELLY_WIKI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("JELLY_BELLY_WIKI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("JELLY_BELLY_WIKI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewJellyBellyWikiSDK(opts)
