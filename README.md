...
API for Jelly Belly beans, facts, recipes, combinations, and history sourced from the official Jelly Belly website.

Learn more about Jelly Belly Wiki at [jellybellywikiapi.onrender.com](https://jellybellywikiapi.onrender.com).

This is an unofficial SDK for the Jelly Belly Wiki public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Bean, Combination, Fact, History and Recipe — that you
...
const items = await client.Bean().list()
```
...
| --- | --- | --- |
| **Bean** | The Bean entity (list, load). | `/beans` |
| **Combination** | The Combination entity (list). | `/combinations` |
| **Fact** | The Fact entity (list). | `/facts` |
| **History** | The History entity (list). | `/history` |
| **Recipe** | The Recipe entity (list). | `/recipes` |
...
$beans = $client->Bean()->list();
print_r(array_map(fn($item) => $item->data_get(), $beans));
