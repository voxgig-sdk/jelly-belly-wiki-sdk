...
func TestBeanDirect(t *testing.T) {
	t.Run("direct-list-bean", func(t *testing.T) {
		setup := beanDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-bean", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-bean", func(t *testing.T) {
		setup := beanDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-bean", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type beanDirectSetupResult struct {
	client *sdk.JellyBellyWikiSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func beanDirectSetup(mockres any) *beanDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"JELLY_BELLY_WIKI_TEST_BEAN_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["JELLY_BELLY_WIKI_TEST_BEAN_ENTID"]; ok {
		...
		return &beanDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &beanDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
