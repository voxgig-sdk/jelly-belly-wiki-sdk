...
import (
	...
	omni "github.com/voxgig-sdk/jelly-belly-wiki-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.JellyBellyWikiError); is {
				out := map[string]any{
					"name":    "JellyBellyWikiError",
...
func omniCtx(arg any, client *sdk.JellyBellyWikiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.JellyBellyWikiSDK); is {
			client = live
