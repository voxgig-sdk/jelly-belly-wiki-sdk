...
func TestFactEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Fact(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Fact(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JELLY_BELLY_WIKI_TEST_FACT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func factBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JELLY_BELLY_WIKI_TEST_FACT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JELLY_BELLY_WIKI_TEST_FACT_ENTID"])
