...
class TestCombinationDirect:

    def test_should_direct_list_combination(self):
        setup = _combination_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-combination", "live" if setup["live"] else "unit")
        if _skip:
...
def _combination_direct_setup(mockres):
