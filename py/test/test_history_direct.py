...
class TestHistoryDirect:

    def test_should_direct_list_history(self):
        setup = _history_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-history", "live" if setup["live"] else "unit")
        if _skip:
...
def _history_direct_setup(mockres):
