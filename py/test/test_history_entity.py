    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set JELLYBELLYWIKI_TEST_HISTORY_ENTID JSON to run live")
        client = setup["client"]
        ...
        history_ref01_list_result, err = history_ref01_ent.list(history_ref01_match, None)
        assert err is None
...
def _history_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("JELLYBELLYWIKI_TEST_HISTORY_ENTID"))
