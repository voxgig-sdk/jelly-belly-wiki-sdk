...
class TestRecipeDirect:

    def test_should_direct_list_recipe(self):
        setup = _recipe_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-recipe", "live" if setup["live"] else "unit")
        if _skip:
...
def _recipe_direct_setup(mockres):
