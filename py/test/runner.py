# JellyBellyWiki SDK test runner
...
class JellyBellyWikiTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JellyBellyWikiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JellyBellyWikiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JellyBellyWikiTestRunner.getenv("JELLY_BELLY_WIKI_TEST_LIVE")
        override = JellyBellyWikiTestRunner.getenv("JELLY_BELLY_WIKI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JellyBellyWikiTestRunner.getenv(key)
        ...
        explain = JellyBellyWikiTestRunner.getenv("JELLY_BELLY_WIKI_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if JellyBellyWikiTestRunner._test_control is not None:
            return JellyBellyWikiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JellyBellyWikiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JellyBellyWikiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JellyBellyWikiTestRunner.load_test_control()
...
def load_env_local():
    JellyBellyWikiTestRunner.load_env_local()


def env_override(m):
    return JellyBellyWikiTestRunner.env_override(m)


def entity_data(v):
    return JellyBellyWikiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JellyBellyWikiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JellyBellyWikiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JellyBellyWikiTestRunner.load_test_control()


def live_client_options():
    return JellyBellyWikiTestRunner.live_client_options()


def live_delay_ms():
    return JellyBellyWikiTestRunner.live_delay_ms()
