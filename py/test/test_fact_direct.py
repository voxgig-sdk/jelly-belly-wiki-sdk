...
class TestFactDirect:

    def test_should_direct_list_fact(self):
        setup = _fact_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-fact", "live" if setup["live"] else "unit")
        if _skip:
...
def _fact_direct_setup(mockres):
