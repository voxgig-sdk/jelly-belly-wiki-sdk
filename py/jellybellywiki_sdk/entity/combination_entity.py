...
class CombinationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CombinationEntity(self._client, opts)
    ...
    def match_get(self) -> Combination:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Combination]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Combination().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
