...
class RecipeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RecipeEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Recipe]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Recipe().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
