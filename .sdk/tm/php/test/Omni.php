...
//    only - a typed JellyBellyWikiContext there would make every ctx
...
final class JellyBellyWikiOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new JellyBellyWikiContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new JellyBellyWikiSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new JellyBellyWikiResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new JellyBellyWikiError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new JellyBellyWikiResponse($clean['response']);
    ...
    public static function err_from_map($m): ?JellyBellyWikiError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new JellyBellyWikiError(is_string($code) ? $code : '', $msg);
    }
