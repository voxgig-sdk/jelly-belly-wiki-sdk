...
// JellyBellyWiki SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class JellyBellyWikiTestFeature extends JellyBellyWikiBaseFeature
{
    ...
    public function init(JellyBellyWikiContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (JellyBellyWikiContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = JellyBellyWikiParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (JellyBellyWikiContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(JellyBellyWikiContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = JellyBellyWikiParam::call($ctx, $k);
