...
// JellyBellyWiki SDK retry feature
...
class JellyBellyWikiRetryFeature extends JellyBellyWikiBaseFeature
{
    ...
    public function init(JellyBellyWikiContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (JellyBellyWikiContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(JellyBellyWikiContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(JellyBellyWikiContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
